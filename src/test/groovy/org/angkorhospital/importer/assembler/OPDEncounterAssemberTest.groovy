...
class OPDEncounterAssemberTest  extends BaseContextSensitiveTest {
    ...
    def sample = OPDEncounterSourceTest.getSampleValuesString();
    def mySource;
	...
	def values = OPDEncounterSourceTest.getSampleValues();
	values["DidNotWait"] = "1";
	values["Admitted To"] = "ER";
	mySource = new OPDEncounterSource(new StringReader(
		OPDEncounterSourceTest.getSampleValuesString(values)));
	mySource.next();
	assembler.setSource(mySource);
